...
class ChannelStripComponent(ControlSurfaceComponent):
    ...
    def __init__(self):
        ...
        def make_button_slot(name):
            return self.register_slot(None, getattr(self, f'_{name}_value'), u'value')
    ...
    def _connect_parameters(self):
        ...
        if self._send_controls != None:
            for index, send_control in enumerate(self._send_controls):
    ...
    def _select_value(self, value):
        ...
        if (
            self.is_enabled()
            and self._track != None
            and (value != 0 or not self._select_button.is_momentary())
            and self.song().view.selected_track != self._track
        ):
    ...
    def _mute_value(self, value):
        ...
        if (
            self.is_enabled()
            and self._track not in [None, self.song().master_track]
            and (not self._mute_button.is_momentary() or value != 0)
        ):
    ...
    def _solo_value(self, value):
        ...
        if self.is_enabled() and self._track not in [
            None,
    ...
    def _crossfade_toggle_value(self, value):
        ...
        if (
            self.is_enabled()
            and self._track != None
            and (value != 0 or not self._crossfade_toggle.is_momentary())
        ):
