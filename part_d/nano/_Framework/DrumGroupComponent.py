...
class DrumGroupComponent(SlideComponent, Slideable):
    ...
    def contents(self, index):
        if drum := self._drum_group_device:
    ...
    @property
    def width(self):
        return self.drum_matrix.width or 4

    @property
    def height(self):
        return self.drum_matrix.height or 4
    ...
    def visible_drum_pads(self):
        if self._visible_drum_pads and self._all_drum_pads:
            if first_pad := first(self._visible_drum_pads):
    ...
    def _update_led_feedback(self):
        if self._drum_group_device:
            ...
            for button in self.drum_matrix:
                if pad := self._coordinate_to_pad_map.get(button.coordinate, None):
    ...
    def _update_pad_led(self, pad, button, soloed_pads):
        ...
        if pad == self._selected_drum_pad:
            ...
            elif soloed_pads and not pad.solo or not soloed_pads or pad.mute:
    ...
    def _set_non_pad_translated_identifiers(self):
        if visible_drum_pads := self.visible_drum_pads:
