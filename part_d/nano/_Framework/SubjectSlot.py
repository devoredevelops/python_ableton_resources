...
def subject_add_event(cls, event_name_or_event):
    ...
    signal_attr = f'_{event.name}_signal'
    ...
    @monkeypatch(cls, f'{event.name}_has_listener', **kwargs)
    def has_method(self, slot):
        return get_signal(self).is_connected(slot)

    @monkeypatch(cls, f'add_{event.name}_listener', **kwargs)
    ...
    @monkeypatch(cls, f'remove_{event.name}_listener', **kwargs)
    def remove_method(self, slot):
        return get_signal(self).disconnect(slot)

    @monkeypatch(cls, f'notify_{event.name}', **kwargs)
    def notify_method(self, *a, **k):
        return (get_signal(self))(*a, **k)

    @monkeypatch(cls, f'clear_{event.name}_listeners', **kwargs)
    def clear_method(self):
        return get_signal(self).disconnect_all()

    @monkeypatch(cls, f'{event.name}_listener_count', **kwargs)
...
class SubjectSlot(Disconnectable):
    ...
    def _check_subject_interface(self, subject):
        if not callable(getattr(subject, f'add_{self._event}_listener', None)):
            raise SubjectSlotError(
                f'Subject {subject} missing "add" method for event: {self._event}'
            )
        if not callable(getattr(subject, f'remove_{self._event}_listener', None)):
            raise SubjectSlotError(
                f'Subject {subject} missing "remove" method for event: {self._event}'
            )
        if not callable(getattr(subject, f'{self._event}_has_listener', None)):
            raise SubjectSlotError(
                f'Subject {subject} missing "has" method for event: {self._event}'
            )

    def connect(self):
        if (self.is_connected or self._subject) is None or self._listener != None:
            add_method = getattr(self._subject, f'add_{self._event}_listener')
    ...
    def soft_disconnect(self):
        if not self.is_connected and self._subject != None or self._listener != None:
            all_args = tuple(self._extra_args) + (self._listener,)
            remove_method = getattr(self._subject, f'remove_{self._event}_listener')
            try:
    ...
    def is_connected(self):
        ...
        try:
            connected = bool(
                self._subject != None
                and self._listener != None
                and getattr(self._subject, f'{self._event}_has_listener')(
                    *all_args
                )
            )
        except RuntimeError:
