...
def find_nearest_color(rgb_table, src_hex_color):
    ...
    def squared_distance(color):
        return sum(
            (a - b) ** 2
            for a, b in zip(
                hex_to_channels(src_hex_color), hex_to_channels(color[1])
            )
        )
...
class ClipSlotComponent(ControlSurfaceComponent):
    ...
    def _feedback_value(self):
        ...
        if (
            ...
            and self._stopped_value != None
        ):
            return self._stopped_value
        if (
            self._track_is_armed(track)
            and self._clip_slot.has_stop_button
            and self._record_button_value != None
        ):
    ...
    def _on_slot_triggered_changed(self):
        ...
        if (
            ...
            and self._clip_slot.is_triggered
            and self._has_fired_slot
            and self._clip_slot.will_record_on_start
            and self._clip_slot != view.highlighted_clip_slot
        ):
    ...
    def _do_select_clip(self, clip_slot):
        if (
            self._clip_slot != None
            and self.song().view.highlighted_clip_slot != self._clip_slot
        ):
