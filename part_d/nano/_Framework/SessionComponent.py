...
class SessionComponent(CompoundComponent):
    ...
    def _update_stop_all_clips_button(self):
        if button := self._stop_all_button:
    ...
    def _on_next_scene_value(self, value):
        if (
            self.is_enabled()
            and value == 0
            and not self._next_scene_button.is_momentary()
        ):
            selected_scene = self.song().view.selected_scene
    ...
    def _on_prev_scene_value(self, value):
        if (
            self.is_enabled()
            and value == 0
            and not self._prev_scene_button.is_momentary()
        ):
            selected_scene = self.song().view.selected_scene
            all_scenes = self.song().scenes
    ...
    def _on_stop_track_value(self, value, button):
        if self.is_enabled() and value == 0 and not button.is_momentary():
    ...
    def _update_stop_clips_led(self, index):
        ...
        if (
            ...
            if button != None:
                value_to_send = None
                if (
                    track_index < len(tracks_to_use)
                    and tracks_to_use[track_index].clip_slots
                ):
