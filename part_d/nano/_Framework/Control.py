...
class RadioButtonControl(Control):
    ...
    class State(Control.State):
        ...
        def _on_value(self, value, *a, **k):
            if self._notifications_enabled():
                if checked := value or self._is_momentary():
...
class EncoderControl(Control):
    ...
    class State(Control.State):
        ...
        def set_control_element(self, control_element):
            ...
            if (
                control_element
                and old_hasattr(control_element, 'is_pressed')
                and control_element.is_pressed()
            ):
                self._touch_encoder()
...
class ControlList(Control):
    ...
    class State(Control.State):
        ...
        def set_control_element(self, control_elements):
            ...
            if self._dynamic_create and len(control_elements or []) != len(
                self._control_element or []
            ):
    ...
    class State(ControlList.State):
        ...
        def set_control_element(self, control_elements):
            ...
            if old_hasattr(control_elements, 'width') and old_hasattr(control_elements, 'height'):
                ...
                if not self._dynamic_create:
                    control_elements = [row[:self.width] for row in control_elements]
