...
def device_to_appoint(device):
    ...
    if device is None or device.can_have_drum_pads:
        if (
            not device.has_macro_mappings
            and len(device.chains) > 0
            and device.view.selected_chain != None
            and len(device.view.selected_chain.devices) > 0
        ):
...
class DeviceComponent(ControlSurfaceComponent, Subject):
    ...
    def restore_bank(self, bank_index):
        if (
            self._device != None
            and self._is_banking_enabled()
            and self._locked_to_device
            and self._number_of_parameter_banks() > bank_index
            and self._bank_index != bank_index
        ):
        ...
        if self.is_enabled() and self._device != None:
            ...
            if self._parameter_controls != None:
                ...
                if self._bank_name != old_bank_name:
                    self._show_msg_callback(f'{self._device.name} Bank: {self._bank_name}')
    ...
    def _bank_up_value(self, value):
        if (
            self.is_enabled()
            and (not self._bank_up_button.is_momentary() or value != 0)
            and self._device != None
        ):
    ...
    def _bank_down_value(self, value):
        if (
            self.is_enabled()
            and (not self._bank_down_button.is_momentary() or value != 0)
            and self._device != None
            and (self._bank_index is None or self._bank_index > 0)
        ):
            self._bank_name = ''
    ...
    def _bank_value(self, value, button):
        if not self.is_enabled():
            return
        if self._device != None and (not button.is_momentary() or value != 0):
            bank = list(self._bank_buttons).index(button)
            if bank == self._bank_index:
                self._show_msg_callback(f'{self._device.name} Bank: {self._bank_name}')

            elif self._number_of_parameter_banks() > bank:
                self._bank_name = ''
    ...
    def _update_device_bank_nav_buttons(self):
        if (
            self.is_enabled()
            and self._bank_up_button != None
            and self._bank_down_button != None
        ):
            can_bank_up = (
                self._bank_index is None
                or self._number_of_parameter_banks() > self._bank_index + 1
            )
    ...
    def _current_bank_details(self):
        ...
        if banks := self._parameter_banks():
