...
class ModesComponent(CompoundComponent):
    ...
    def selected_groups(self):
        if entry := self._mode_map.get(self.selected_mode, None):
            return entry.groups
        return set()
    ...
    def add_mode(self, name, mode_or_component, toggle_value=False, groups=set(), behaviour=None):
        ...
        self._mode_map[name] = _ModeEntry(mode=mode,
          ...
          momentary_task=task,
          groups=groups)
        button_setter = f'set_{name}_button'
    ...
    def get_mode_groups(self, name):
        return entry.groups if (entry := self._mode_map.get(name, None)) else set()
    ...
    def _update_data_sources(self, selected):
        if self.is_enabled():
            for name, (source, string) in self._mode_data_sources.items():
                source.set_display_string(f'*{string}' if name == selected else string)
