...
class MixerComponent(CompoundComponent):
    ...
    def _bank_up_value(self, value):
        if (
            self.is_enabled()
            and value == 0
            and not self._bank_up_button.is_momentary()
        ):
    ...
    def _bank_down_value(self, value):
        if (
            self.is_enabled()
            and value == 0
            and not self._bank_down_button.is_momentary()
        ):
    ...
    def _next_track_value(self, value):
        if (
            self.is_enabled()
            and value == 0
            and not self._next_track_button.is_momentary()
        ):
            selected_track = self.song().view.selected_track
    ...
    def _prev_track_value(self, value):
        if (
            self.is_enabled()
            and value == 0
            and not self._prev_track_button.is_momentary()
        ):
            selected_track = self.song().view.selected_track
            all_tracks = tuple(self.song().visible_tracks) + tuple(self.song().return_tracks) + (self.song().master_track,)
