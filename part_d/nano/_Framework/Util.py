...
def in_range(value, lower_bound, upper_open_bound):
    return (
        value >= lower_bound and value < upper_open_bound
...
def to_slice(obj):
    if isinstance(obj, slice):
        return obj
    return slice(obj, obj + 1) if obj != -1 else slice(obj, None)
...
def mixin(*args):
    if len(args) == 1:
        return args[0]
    name = f"Mixin_{'_'.join(cls.__name__ for cls in args)}"
...
class overlaymap(object):
    ...
    def keys(self):
        res = set(chain_from_iterable(self._maps))
        return list(res)
