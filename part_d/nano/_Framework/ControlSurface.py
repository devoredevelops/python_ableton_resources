...
class ControlSurface(Subject, SlotManager):
    ...
    def suggest_map_mode(self, cc_no, channel):
        return next(
            (
                ...
                and control.message_type() == MIDI_CC_TYPE
                and control.message_identifier() == cc_no
                and control.message_channel() == channel
            ),
            -1,
        )
    ...
    def handle_nonsysex(self, midi_bytes):
        ...
        if recipient is not None:
            ...
            self.log_message(f'Got unknown message: {str(midi_bytes)}')
    ...
    def handle_sysex(self, midi_bytes):
        ...
        if result != None:
            ...
            self.log_message(f'Got unknown sysex message: {str(midi_bytes)}')
    ...
    def _set_suppress_rebuild_requests(self, suppress_requests):
        if suppress_requests:
            ...
            if (
                self._suppress_requests_counter == 0
                and self._rebuild_requests_during_suppression > 0
            ):
